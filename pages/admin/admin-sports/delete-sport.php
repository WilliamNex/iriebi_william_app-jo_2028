...
if (!isset($_GET['id_sport'])) {
    ...
    if ($id_sport === false) {
        ...
        try {
            // First, delete related epreuves
            $sqlEpreuve = "DELETE FROM EPREUVE WHERE id_sport = :param_id_sport";
            $statementEpreuve = $connexion->prepare($sqlEpreuve);
            $statementEpreuve->bindParam(':param_id_sport', $id_sport, PDO::PARAM_INT);
            $statementEpreuve->execute();

            // Then, delete the sport
            $sql = "DELETE FROM SPORT WHERE id_sport = :param_id_sport";
